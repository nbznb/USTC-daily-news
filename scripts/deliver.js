...
async function sendTelegram(text, botToken, chatId) {
  ...
  for (const chunk of chunks) {
    ...
    if (!res.ok) {
      ...
      if (error.description && error.description.includes("can't parse")) {
...
async function sendEmail(text, apiKey, toEmail) {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'USTC Daily News <digest@resend.dev>',
      to: [toEmail],
      subject: `USTC Daily News — ${new Date().toLocaleDateString('zh-CN', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
